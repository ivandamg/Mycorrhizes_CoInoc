...
head(Mercator_Mesculenta,5)
Mercator_Mesculenta$Type<-unlist( lapply(lapply(strsplit(as.character(Mercator_Mesculenta[,2],20),split="[.]"),function (x) x[1:2]),function (x) paste(x[1],x[2],sep ="_" )  ))
...
dev.off()
############################################
# functional traits resume table CASSAVA

# only CANB1
only_canb1
only_canb1_v2<-cbind.data.frame(unique(only_canb1),rep("V4",length(unique(only_canb1))),
                                rep("V5",length(unique(only_canb1))),rep("V6",length(unique(only_canb1))))
only_canb1_v2[,2]<-as.vector(only_canb1_v2[,2]);only_canb1_v2[,3]<-as.vector(only_canb1_v2[,3]);only_canb1_v2[,4]<-as.vector(only_canb1_v2[,4])
only_canb1_v2[only_canb1_v2[,1] %in% rownames(V4@.Data[V4@.Data[,3]!=0&V4@.Data[,1]==0&V4@.Data[,2]==0,]),][,2]<-1
only_canb1_v2[!only_canb1_v2[,1] %in% rownames(V4@.Data[V4@.Data[,3]!=0&V4@.Data[,1]==0&V4@.Data[,2]==0,]),][,2]<-0
only_canb1_v2[only_canb1_v2[,1] %in% rownames(V5@.Data[V5@.Data[,3]!=0&V5@.Data[,1]==0&V5@.Data[,2]==0,]),][,3]<-1
only_canb1_v2[!only_canb1_v2[,1] %in% rownames(V5@.Data[V5@.Data[,3]!=0&V5@.Data[,1]==0&V5@.Data[,2]==0,]),][,3]<-0
only_canb1_v2[only_canb1_v2[,1] %in% rownames(V6@.Data[V6@.Data[,3]!=0&V6@.Data[,1]==0&V6@.Data[,2]==0,]),][,4]<-1
only_canb1_v2[!only_canb1_v2[,1] %in% rownames(V6@.Data[V6@.Data[,3]!=0&V6@.Data[,1]==0&V6@.Data[,2]==0,]),][,4]<-0
colnames(only_canb1_v2)<-c("gene","COL2215","BRA337","CM4574-7")
only_canb1_v2<-merge(only_canb1_v2,Mercator_Mesculenta,by="gene")
only_canb1_v2<-only_canb1_v2[!duplicated(only_canb1_v2$gene),]

Total_seq<-sign_YUCA
Total_seq$gene<-rownames(sign_YUCA)
Total_seq<-merge(Total_seq,Mercator_Mesculenta,by="gene")
Total_seq<-Total_seq[!duplicated(Total_seq$gene),]


Sign_seq<-sign_YUCA[sign_YUCA$adj.P.Val<0.05,]
Sign_seq$gene<-rownames(sign_YUCA[sign_YUCA$adj.P.Val<0.05,])
Sign_seq<-merge(Sign_seq,Mercator_Mesculenta,by="gene")
Sign_seq<-Sign_seq[!duplicated(Sign_seq$gene),]

funct_table_VF<-list()
for (i in 1:length(levels(droplevels(as.factor(only_canb1_v2$Type)))) ) {

all_CANB1<-droplevels(only_canb1_v2[grep(levels(as.factor(only_canb1_v2$Type))[i],only_canb1_v2$Type),c(1,2,3,4,8)])
all_CANB1[,2]<-as.numeric(all_CANB1[,2]);all_CANB1[,3]<-as.numeric(all_CANB1[,3]);all_CANB1[,4]<-as.numeric(all_CANB1[,4])
as.matrix(all_CANB1)
# final results all_CANB1
all_CANB1_nom<-levels(as.factor(only_canb1_v2$Type))[i]
all_CANB1_var<-c(sum(all_CANB1[,2]),sum(all_CANB1[,3]),sum(all_CANB1[,4]) )
all_CANB1_total<-sum(c(sum(all_CANB1[,2]),sum(all_CANB1[,3]),sum(all_CANB1[,4]) ))
# ALL sequenced

Total_Seq_2<-length(grep(levels(as.factor(only_canb1_v2$Type))[i],Total_seq$Type))
# Different from control
Sign_seq_2<-length(grep(levels(as.factor(only_canb1_v2$Type))[i],Sign_seq$Type))


funct_table_VF[[i]]<-cbind.data.frame(all_CANB1_nom,Total_Seq_2,Sign_seq_2,all_CANB1_total,all_CANB1_var[1],all_CANB1_var[2],all_CANB1_var[3])

}
funct_table_VF[[3]]

funct_table_VF2<-data.frame(matrix(unlist(funct_table_VF), nrow=length(levels(droplevels(as.factor(only_canb1_v2$Type)))), byrow=T))
funct_table_VF2[,1]<-levels(droplevels(as.factor(only_canb1_v2$Type)))
colnames(funct_table_VF2)<-c("Gene","Total_dataset","Total_significant","Total_diff_CANB1","Diff_COL2215",
                             "Diff_BRA337","Diff_CM4574-7")
head(funct_table_VF2)

write.table(funct_table_VF2,'~/Google Drive/Doctorat_shared_unil/RNA-seq/Results/coinoculation_manuscript/V5_coinoc/Table_function_CANB1_cassava.txt',
...
boxplot(NORM_EXP_Y[,grep("Manes.S086300",colnames(NORM_EXP_Y))]~interaction(tratamiento,cultivar),las=2)
dim(NORM_EXP_Y)
Results_A_vf2[Results_A_vf2$gene=="Manes.S086300",]
...
########################################################################################
############################################
# functional traits resume table AMF

# only CANB1
only_canb1<-c(rownames(V4@.Data[V4@.Data[,1]!=0&V4@.Data[,2]!=0,]),
              rownames(V5@.Data[V5@.Data[,1]!=0&V5@.Data[,2]!=0,]),
              rownames(V6@.Data[V6@.Data[,1]!=0&V6@.Data[,2]!=0,]))
only_canb1_v2<-cbind.data.frame(unique(only_canb1),rep("V4",length(unique(only_canb1))),
                                rep("V5",length(unique(only_canb1))),rep("V6",length(unique(only_canb1))))
...
only_canb1_v2[only_canb1_v2[,1] %in% rownames(V4@.Data[V4@.Data[,2]!=0&V4@.Data[,1]!=0,]),][,2]<-1
only_canb1_v2[!only_canb1_v2[,1] %in% rownames(V4@.Data[V4@.Data[,2]!=0&V4@.Data[,1]!=0,]),][,2]<-0
only_canb1_v2[only_canb1_v2[,1] %in% rownames(V5@.Data[V5@.Data[,1]!=0&V5@.Data[,2]!=0,]),][,3]<-1
only_canb1_v2[!only_canb1_v2[,1] %in% rownames(V5@.Data[V5@.Data[,1]!=0&V5@.Data[,2]!=0,]),][,3]<-0
#only_canb1_v2[only_canb1_v2[,1] %in% rownames(V6@.Data[V6@.Data[,1]!=0&V6@.Data[,2]!=0,]),][,4]<-1
only_canb1_v2[!only_canb1_v2[,1] %in% rownames(V6@.Data[V6@.Data[,1]!=0&V6@.Data[,2]!=0,]),][,4]<-0

colnames(only_canb1_v2)<-c("gene","COL2215","BRA337","CM4574-7")
only_canb1_v2<-merge(only_canb1_v2,Mercator_Rirregularis,by="gene")
only_canb1_v2<-only_canb1_v2[!duplicated(only_canb1_v2$gene),]
head(only_canb1_v2)

Total_seq<-sign_AMF
Total_seq$gene<-rownames(sign_AMF)
Total_seq<-merge(Total_seq,Mercator_Rirregularis,by="gene")
Total_seq<-Total_seq[!duplicated(Total_seq$gene),]


Sign_seq<-sign_AMF[sign_AMF$adj.P.Val<0.05,]
Sign_seq$gene<-rownames(sign_AMF[sign_AMF$adj.P.Val<0.05,])
Sign_seq<-merge(Sign_seq,Mercator_Rirregularis,by="gene")
Sign_seq<-Sign_seq[!duplicated(Sign_seq$gene),]
...
write.table(funct_table_VF2,'~/Google Drive/Doctorat_shared_unil/RNA-seq/Results/coinoculation_manuscript/V5_coinoc/Table_function_CANB1_AMF.txt',
            quote=FALSE,sep='\t',col.names = T, row.names = T)
############################################
########################################################################################
...
for (i in 1:dim(genes_YUCA_compe2)[1]) {
  plot(genes_YUCA_compe2[i,][grep("CTRL",names(genes_YUCA_compe2[i,]),invert=T)],phen$total,
       ylab=paste("gene-expression\n",rownames(genes_YUCA_compe2)[i]))}
######################################################
######################################################
...
####
# test genes
genes_AMF_compe2<-DGE_AMF$counts[rownames(DGE_AMF$counts) %in% "g7953.t1", ] 

stDevs <-tapply(genes_AMF_compe2,interacts,sd)
means<-tapply(genes_AMF_compe2,interacts,mean)
mp<-barplot(tapply(genes_AMF_compe2,interacts,mean),ylim=c(0,max(means + stDevs)*1.1),
...
##### Function Mercator tools
         ...
         sign_YUCA2<-sign_YUCA[rownames(sign_YUCA) %in% Coinoc_higher,]

sign_YUCA2$gene<-rownames(sign_YUCA[rownames(sign_YUCA) %in% Coinoc_higher,])
sign_YUCA2<-merge(sign_YUCA2,Mercator_Mesculenta,by="gene")
colnames(sign_YUCA2)
write.table( sign_YUCA2[,c(1:10,14,16:17)],'~/Google Drive/Doctorat_shared_unil/RNA-seq/Results/coinoculation_manuscript/highcoinoc_cassava.txt',
             quote=FALSE,sep='\t',col.names = T, row.names = T)
...
# individual genes
write.table(All_TOP_genes3[,c(1:10,14,15,16,17)],'~/Google Drive/Doctorat_shared_unil/RNA-seq/Results/coinoculation_manuscript/V4_coinoc /Supplementary_table_YUCA_PHENO.txt',quote=FALSE,sep='\t',
            col.names = T, row.names = T)
...
write.table(cbind.data.frame(All_TOP_genes3[,c(1)],correlacion,All_TOP_genes3[,c(2:10,14,15,16,17)]),'~/Google Drive/Doctorat_shared_unil/RNA-seq/Results/coinoculation_manuscript/V4_coinoc /Supplementary_table_YUCA_PHENO.txt',quote=FALSE,sep='\t',
            col.names = T, row.names = T)
...
pt1<-t(DGE_YUCA_N$E[rownames(DGE_YUCA_N$E) %in%  "Manes.01G111700",])
...
write.table(cbind.data.frame(correlacion,All_TOP_genes3[,c(2:7,11,13,14)]),'~/Google Drive/Doctorat_shared_unil/RNA-seq/Results/coinoculation_manuscript/V4_coinoc /Supplementary_table_AMF_PHENO.txt',quote=FALSE,sep='\t',
            col.names = T, row.names = T)
...
# distance matrix croll Msat

croll<- read.table("~/Google Drive/Doctorat_shared_unil/RNA-seq/Results/coinoculation_manuscript/V6_coinoc/crollmsat.txt",h=T)
croll
croll2<-dist(croll[,2:13])
names(croll2)<-croll[,1]
